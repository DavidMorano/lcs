.\"_ ECE3484 Greedy algorithm writeup 
.\"_
.nr N 2
.\"_
...
consider one to illustrate the exponential nature of it.
If we try to extend the Greedy algorithm as implemented
...
due to cleverness in pruning search times), could miss
finding the LCS of the problem.
.\_"
.H 1 "Program Restrictions and Input Format"
.P
The input format for strings is very flexible.
There are no restrictions on the number of files that can be
used to contain the problem instances.  Each file can also contain
any
number of input strings.  Also there is no restriction on 
the length of any of the input strings either.  All strings
found 
(each on a separate line of a file) 
in all files are cnosidered a single problem instance.
All problem instances must be loaded into processor main memory
so this places a maximum
restriction on the total size of the problem instance.
Obviously, this is a practical restriction.
.\"_
.\"_
.\"_
