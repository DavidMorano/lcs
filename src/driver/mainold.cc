...
extern "C" int	optmatch() ;
extern "C" int	procfile() ;
extern "C" int	random3() ;
...
static void	print_string(bfile *,cchar *,int) ;
...
int main(int argc,cchar **argv,cchar **envv)
{
	...
	BA_NUM		bitarray_support ;
	OURSTRS		strings	s, s_out ;
	OURSTRS		*rp ;
...
/* perform initialization processing */
	...
	ba_numprepare(&bitarray_support) ;

	gp->iterations = 0 ;
	strings_start(&s,&bitarray_support) ;
...
/* read in the input files */
	...
	if (g.verboselevel > 2) {
	    ...
	    for (i = 0 ; vechand_get(&s.vl,i,&rp) >= 0 ; i += 1) {
	        if (rp == NULL) continue ;
	...
	strings_start(&s_out,&bitarray_support) ;
...
void print_string(bfile *fp,cchar *s,int len)
{
	int	plen ;
