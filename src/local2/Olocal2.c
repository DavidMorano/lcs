...
/* loop through all characters of the chosen "expand" string, finding a CS */
	...
	search_len = MIN(esep->len - lcs_len - 1,esep->len) ;
...
#if	F_DEBUG
	    if (gp->debuglevel > 1)
	        eprintf("extend: entered, cslen=%d, csj=%d\n",
			cslen,csj) ;
#endif
